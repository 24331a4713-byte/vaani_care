...
export default function HospitalMap() {
  ...
  const getUserLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) =>
        fetchFacilities(pos.coords.latitude, pos.coords.longitude, severity),
  ...
  const fetchFacilities = async (lat, lon, sev = severity) => {
    setLocation({ lat, lon });
    try {
      const res = await fetch(`${API}/nearest-facility`, {
        method: "POST",
        ...
        {SEVERITY_OPTIONS.map((s) => (
          <button
            ...
            onClick={() => {
              ...
              if (location)
                fetchFacilities(location.lat, location.lon, s.value);
            }}
            style={{
              ...styles.filterBtn,
